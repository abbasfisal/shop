...
func (zarinpal *Zarinpal) NewPaymentRequest(TomanAmount int, callbackURL, description, email, mobile string) (paymentURL, authority string, statusCode int, err error) {
	if TomanAmount < 1 {
...
// based on their documentation placed in
// https://github.com/ZarinPal-Lab/Documentation-PaymentGateway/archive/master.zip
func (zarinpal *Zarinpal) PaymentVerification(TomanAmount int, authority string) (verified bool, refID string, statusCode int, err error) {
	if TomanAmount <= 0 {
		err = errors.New("amount must be a positive number")
		return
	}
