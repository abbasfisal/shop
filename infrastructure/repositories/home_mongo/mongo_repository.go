...
func (m MongoHomeRepository) GetProduct(c *gin.Context, productSku string, productSlug string) (map[string]interface{}, []entities.MongoProductRecommendation, error) {
	...
	if FindProductErr != nil {
		if FindProductErr == mongo.ErrNoDocuments {
			...
			return nil, nil, errors.New(domain_err.RecordNotFound)
		}
		return nil, nil, errors.New(domain_err.SomethingWrongHappened)
	}
...
func (m MongoHomeRepository) GetProductByObjectID(c *gin.Context, productObjectID primitive.ObjectID, req requests.AddToCartRequest) (entities.MongoProduct, error) {
	...
	if FindProductErr != nil {
		if FindProductErr == mongo.ErrNoDocuments {
			// محصولی یافت نشد
			fmt.Println("[Error Not Found Document ]-[GetProductByObjectID]:err:", domain_err.RecordNotFound)
			return mongoProduct, errors.New(domain_err.RecordNotFound)
		}
		fmt.Println("[Error]-[GetProductByObjectID]:err:", domain_err.SomethingWrongHappened)

		return mongoProduct, errors.New(domain_err.SomethingWrongHappened)
	}
