...
func CheckCustomerSessionID(c *gin.Context) {
	...
	fmt.Println("--- middleware check customer session id ")
	customer := helpers.CustomerAuth(c)

	if customer.ID > 0 {
		fmt.Println("--- middleware check customer session id was exist")
		c.Redirect(http.StatusFound, "/")
		c.Abort()
		return
	}
	fmt.Println("--- middleware check customer session id was not exist ")
	c.Next()
}
