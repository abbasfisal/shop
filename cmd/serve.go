...
func RunServe() {
	...
	go RunWorker(ctx, dependencies, em)    // run Asynq worker
	go RunScheduler(ctx, dependencies, em) // run Asynq Schedule
...
func setupRoutes(ctx context.Context, r *gin.Engine, dep *bootstrap.Dependencies, em *events.EventManager) {
	...
	r.Static("/assets", filepath.Join(wd, "public"))

	r.StaticFile("/favicon.ico", filepath.Join(wd, "public", "shop", "img", "seller-logo.png"))
...
var serveCmd = &cobra.Command{
	Use:   "serve",
	Short: "run http server + worker + scheduler",
