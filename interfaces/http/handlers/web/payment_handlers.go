...
func (p PublicHandler) Payment(c *gin.Context) {
	...
	if paymentError != nil {
		...
		if errors2.Is(paymentError, domain_err.InternalServerErr) {

			sessions.Set(c, "message", domain_err.InternalServerError)
		}
		if errors2.Is(paymentError, domain_err.OutOfStock) {
...
func (p PublicHandler) VerifyPayment(c *gin.Context) {
	...
	if authority == "" {
		response.CustomerRender(c, http.StatusPermanentRedirect, "404", gin.H{})
		return
	}
	//if status == "NOK" {
	...
	if err != nil {
		log.Println("[zarinpal err]:", err)
		response.CustomerRender(c, http.StatusInternalServerError, "500", gin.H{})
		return
	}
	...
	if verified {
		response.CustomerRender(c, http.StatusOK, "shopping_complete_buy",
			gin.H{
	...
	if !verified || vErr != nil {
		...
		response.CustomerRender(c, http.StatusOK, "shopping_no_complete_buy",
			gin.H{
				"ORDER_NUMBER": order.OrderNumber,
			})
		return
