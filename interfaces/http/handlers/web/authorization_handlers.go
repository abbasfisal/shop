...
func (p PublicHandler) ShowLogin(c *gin.Context) {
	...
	response.CustomerRender(c, 200, "customer_login", gin.H{
		"TITLE": "اسم فروشگاه",
		"data":  "data",
	})
}
...
func (p PublicHandler) ShowVerifyOtp(c *gin.Context) {
	...
	response.CustomerRender(c, 200, "customer_verify_phone_number", gin.H{
...
func (p PublicHandler) PostLogin(c *gin.Context) {
	...
	if otpErr.Code > 0 {
		if otpErr.Code == domain_err.OTPTooSoonCode {
			sessions.Set(c, "message", domain_err.OTPRequestTooSoon)
