...
func (a *AdminHandler) IndexProduct(c *gin.Context) {
	...
	response.Render(c, http.StatusFound, "modules/admin/html/admin_index_product", gin.H{
		"TITLE":    "لیست محصولات",
		"PRODUCTS": products,
	})
	return
...
func (a *AdminHandler) CreateProduct(c *gin.Context) {
	...
	response.Render(c, http.StatusFound, "modules/admin/html/admin_create_product", gin.H{
...
func (a *AdminHandler) StoreProduct(c *gin.Context) {
	...
	if !IsUnique {
		errors.Init()
		errors.Add("sku", domain_err.MustBeUnique)
		sessions.Set(c, "errors", errors.ToString())
	...
	if imagesFile == nil {
		errors.Init()
		errors.Add("images", domain_err.IsRequired)
		sessions.Set(c, "errors", errors.ToString())
	...
	for _, image := range imagesFile {
		...
		if !ok {
			errors.Init()
			errors.Add("images", domain_err.MustBeImage)
			sessions.Set(c, "errors", errors.ToString())
...
func (a *AdminHandler) ShowProduct(c *gin.Context) {
	...
	response.Render(c, http.StatusFound, "admin_show_product",
		gin.H{
			"TITLE":   "نمایش محصول",
			"PRODUCT": selectedP,
		},
...
func (a *AdminHandler) EditProduct(c *gin.Context) {
	...
	response.Render(c, http.StatusFound, "modules/admin/html/admin_edit_product",
		gin.H{
			"TITLE":           "ویرایش محصول",
			"PRODUCT":         productShow,
